...
const isFieldsetValueEnd = (value?: string): boolean => value === '--- /';

const isParagraph = (node: Node): node is Paragraph =>
  node.type === 'paragraph';

const isFieldsetEnd = (node: Node): boolean =>
...
const isFieldsetValueStart = (value?: string): boolean =>
  ...
  value.length > 4 &&
  value.startsWith('--- ') &&
  !isFieldsetValueEnd(value);

const isFieldsetStart = (node: Node): boolean =>
...
export const parseFieldsets = (tree: Root) => {
  ...
  visitParents(
    tree,
    isFieldsetStart,
    (start: Paragraph, ancestors: Parent[]) => {
      ...
      const fieldset = {
        type: 'fieldset',
        children: [getLegend(start), ...between],
        data: {
          hName: 'fieldset'
        }
