 ...
 */
export const getListInput = (
  schema: CheckboxSchema | RadioSchema,
  items: Choice[],
  type: 'radio' | 'checkbox'
): HastElements => {
  ...
  return getField(
    schema,
    h(
      `.form-choices.${type}${variant}`,
      items.map((choice: Choice) => {
        ...
        return h(`.form-choices-item.${type}${variant}`, [
          ...
          h(
            'label',
            { for: id, unselectable: isButton.toString() },
...
export const getOptions = (
  items: Array<Choice | ChoiceGroup>
): HastElements[] =>
  items.map((item) => {
    if ('enum' in item) {
      // @ts-expect-error
